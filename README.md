 ...
 <!--第一部分内容-->
  ...
  <ul class="num" id="idNum">
    ...
    <li>3</li>
          ...
          <div id="notice">
          	<h1><a href="{APP_PATH}index.php?m=content&c=index&a=lists&catid=22"><img src="{IMG_PATH}/hbschool/30.gif" width="30" height="9" /></a></h1>
            <ul>
                {pc:content action="lists" catid="22" order="inputtime DESC" num="8"}
                   {loop $data $v}
                        <li><a href="{$v['url']}">{$v['title']}</a><span class="newsdate">{date('Y-m-d',$v['inputtime'])}</span></li>
                   {/loop}
                {/pc} 
	  ...
	  <div id="no2">
        	<div class="news_content">
            	<h1><img src="{IMG_PATH}/hbschool/news_jy_title.gif" width="480" height="40" usemap="#Map" border="0" alt="教研网" />
                 ...
                 <ul>
                 {pc:content action="lists" catid="13" order="inputtime DESC" num="8"}
                   {loop $data $v}
                        <li><a href="{$v['url']}">{$v['title']}</a><span class="newsdate">{date('Y-m-d',$v['inputtime'])}</span></li>
                   {/loop}
                {/pc} 
            ...
            <div class="news_content" style="margin-left:45px; _margin-left:25px">
                ...
                <ul>
                {pc:content action="lists" catid="12" order="inputtime DESC" num="8"}
                   {loop $data $v}
                        <li><a href="{$v['url']}">{$v['title']}</a><span class="newsdate">{date('Y-m-d',$v['inputtime'])}</span></li>
                   {/loop}
                {/pc} 
